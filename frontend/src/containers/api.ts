...
export const pdfApi = {
	lock: async ({ file, password }: { file: RcFile; password: string }) => {
	...
	unlock: async ({ file, password }: { file: RcFile; password: string }) => {
		const formData = new FormData();
		formData.append("file", file);
		formData.append("password", password);
	...
	download: ({ file_data, filename }: { file_data: string; filename: string }) => {
		try {
